...
from spfx.format import MAGIC, CHUNK_CHECKSUM, NODE_TOOL_CALL, NODE_TOOL_RESULT
from spfx.reader import SPIFChecksumError, SPIFFormatError
from spfx.streaming import SPIFStreamWriter, SPIFStreamReader, iter_events
from spfx.types import Delta, NodeRef, Signature
...
RUST_VIEWER = Path(__file__).parent.parent.parent / "sif-rust" / "target" / "debug" / "spfx-viewer"
...
@pytest.mark.skipif(not RUST_VIEWER.exists(), reason="Rust viewer not built")
class TestCrossLanguageCompat:
    """Python writes a document → Rust viewer must exit 0 and report a model name."""

    def _rust_reads(self, doc: SPIFDocument) -> str:
        """Encode doc in Python, run Rust viewer, return stdout."""
        ...
        try:
            result = subprocess.run(
                [str(RUST_VIEWER), path],
                capture_output=True, text=True, timeout=10,
            )
            assert result.returncode == 0, f"Rust viewer failed: {result.stderr}"
            return result.stdout
        finally:
            os.unlink(path)

    def test_minimal_doc(self):
        doc = SPIFDocument(payload=[_n(0, "hello from Python")])
        out = self._rust_reads(doc)
        assert "SPIF DOCUMENT" in out

    def test_full_doc_with_provenance(self):
        doc = SPIFDocument(payload=[_n(i) for i in range(5)], provenance=_prov())
        out = self._rust_reads(doc)
        assert "SPIF DOCUMENT" in out

    def test_compressed_doc(self):
        nodes = [_n(i, val="x" * 100) for i in range(20)]
        ...
        with tempfile.NamedTemporaryFile(suffix=".spfx", delete=False) as f:
            ...
            path = f.name
        try:
            result = subprocess.run([str(RUST_VIEWER), path],
                                    capture_output=True, text=True, timeout=10)
            assert result.returncode == 0, f"Rust failed on compressed doc: {result.stderr}"
        finally:
            os.unlink(path)

    def test_doc_with_trace(self):
        steps = [TraceStep(id=f"s{i}", type="inference", content=f"step {i}",
                           confidence=_d(), deps=[f"s{i-1}"] if i > 0 else [])
                 for i in range(5)]
        doc = SPIFDocument(payload=[_n(0)], trace=steps, provenance=_prov())
        out = self._rust_reads(doc)
        assert "SPIF DOCUMENT" in out

    def test_binary_identical_to_self(self):
