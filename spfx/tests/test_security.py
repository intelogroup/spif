...
from spfx.format import CHUNK_SIGNATURE, CHUNK_MULTISIG, CHUNK_CHECKSUM, MAGIC
...
class TestMalformedCBOR:
    ...
    def _inject_and_decode(self, chunk_type: int) -> None:
        from spfx.format import CHUNK_SIGNATURE
        doc = _minimal_doc()
        # Add trace + signature so those chunks exist
        from spfx import TraceStep
        doc.trace = [
...
class TestTimingSafety:
    def test_hmac_imported_in_reader(self):
        ...
        """
        import spfx.reader as reader_module
        import inspect
...
class TestCryptoModule:
    ...
    def test_rotate_key_returns_valid_record(self):
        ...
        import warnings
        from spfx.crypto import rotate_key, derive_key_from_mnemonic
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", UserWarning)
    ...
    def test_content_id_stable(self):
        ...
        from spfx.writer import compute_content_id
        doc = _minimal_doc()
