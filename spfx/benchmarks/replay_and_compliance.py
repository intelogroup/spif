...
Usage: python spfx/benchmarks/replay_and_compliance.py
"""
...
from spfx import SPIFDocument, Node, Provenance, SPIFWriter, ReplayGuard, SPIFReplayError
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        path = Path(td) / "doc.spfx"
        path.write_bytes(SPIFWriter().encode(doc))
        result = subprocess.run(
            [sys.executable, "-m", "spfx.cli", "inspect", str(path), "--json"],
