...
6. chained    — two calls linked via context_ref (multi-turn provenance chain)

Cross-language decode coverage (Python-written SPIF read by another
implementation) lives in test_compat_fixtures.py against compat/sif_reader.ts
— there is no standalone native viewer/desktop app anymore; browser-based
verification is at verify/ (spif.dev/verify).
"""
