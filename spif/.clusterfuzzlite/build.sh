...
# package we just built is actually importable before compiling fuzzers
# against it.
python3 -c "import spif" || { echo "FATAL: spif not importable after pip3 install ." >&2; exit 1; }
...
for pkg in "$OUT"/*_fuzzer.pkg; do
  ...
  if ! "$pkg" -runs=1 "$seed" >/tmp/smoketest.log 2>&1; then
    echo "FATAL: $pkg failed to run against a real seed input:" >&2
    cat /tmp/smoketest.log >&2
    exit 1
  fi
  chmod -x "$pkg"
done
